	...
	internal class DensityGenerator
	{
		...
		public void CreateCharts()
		{
			if (isInit)
			{
				...
				{
					double[] densities = GetDensities(pair[0], pair[1], 1000);
					//CreateChart($"Chart for M = {pair[0]}, Sigma = {pair[1]}", densities);
		...
		public void CreateChart()
		{
			if (isInit)
			{
				double[] values = GetRandomByReverseFormula(10, 2, 1000);
				var density_chart = new ChartPreview($"Values chart for M = 10, Sigma = 2", values);
				density_chart.ShowDialog();
				density_chart.Save();
			}
			else
		...
		private double[] GetDensities(double m, double sigma, int amount)
		{
			var densities = new List<double>();
			for (int i = 0; i < 20; i++)
			{
